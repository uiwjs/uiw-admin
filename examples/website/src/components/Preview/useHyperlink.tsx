    ...
    if (dom) {
      const clickFn = (event: MouseEvent) => {
        ...
        if (target.className.indexOf('toc-link') > -1) {
          (
            target.parentElement?.childNodes as unknown as HTMLAnchorElement[]
          ).forEach((item) => {
      ...
      const scrollEndFn = () => {
        ...
        const anchor = dom.getElementsByClassName(
          'anchor',
        ...
        Array.from(anchor).forEach((item, idx) => {
          ...
          if ($main[0].scrollTop - item.offsetTop > -1) {
            hash = new URL(item.href || '').hash.replace(/^#/, '');
          }
        });
        // searchParams.set('_id', decodeURIComponent(hash));
        ...
        const tocLink = document.querySelector(
          `a[href="#${decodeURIComponent(hash)}"].toc-link`,
        ) as HTMLAnchorElement;
        if (tocLink) {
          (
            tocLink.parentElement?.childNodes as unknown as HTMLAnchorElement[]
          ).forEach((item) => {
            item.classList.remove('active');
          });
      ...
      const callback = () => {
        ...
        $main[0].addEventListener('scroll', scrollFn);
        $main[0].addEventListener('scroll', debounce<any>(scrollEndFn, 50));
      };
      // 创建一个观察器实例并传入回调函数
