...
const RenderField = (props: any) => {
  ...
  const errObj = errorFields.find(
    (err: { [key: string]: any }) => err.name === dataPath,
  );
  ...
  // 是否以readType Descriptions的方式预览
  // console.log('readType--->', readType)
  const isDescription = readType === 'description';
  const removeDupErrors = (arr: any[]) => {
  ...
  const messageProps = {
    ...
    schema: _schema,
    displayType,
    softHidden: displayType === 'inline' || softHidden, // 这个是如果没有校验信息时，展示与否
