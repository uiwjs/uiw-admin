...
| authorityJudgment | 否   | `string[] \| RegExp \| ((current: string) => boolean)`  |   | 控制哪些菜单是可以不用登录直接展示的  |
...
⚠️注意：`authorityJudgment` 参数控制哪些菜单是可以不用登录直接展示的，当使用这个的时候的时候注意📢在不登录的情况下配置路由中 `isAuth` 参数无效 ，直接走`authorityJudgment`返回值判断。当登录的情况下，在路由的权限没有存储本地的情况下，才会调用`authorityJudgment`进行判断当前路由权限
